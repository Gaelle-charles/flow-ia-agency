...
import { createContactInputSchema } from "@/lib/contact-schema";
...
export function ProcessIntakeForm() {
  const { form: copy } = useLocalizedContent();
  const { locale } = useI18n();
  const startedAt = useRef(Date.now());
  const [status, setStatus] = useState<FormStatus>({ state: "idle" });

  useEffect(() => setStatus({ state: "idle" }), [locale]);

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = event.currentTarget;
    const data = new FormData(form);
    const parsed = createContactInputSchema(locale).safeParse({
      name: data.get("name"),
    ...
    if (!parsed.success) {
      setStatus({
        state: "error",
        message: parsed.error.issues[0]?.message ?? copy.genericError,
      });
      return;
    }
    ...
    try {
      ...
      if (!result.ok) {
        setStatus({ state: "error", message: copy.serverErrors[result.code] });
        return;
      }
    ...
    <form className="space-y-4" onSubmit={handleSubmit} aria-describedby="contact-privacy-note">
      ...
      <label className="block text-sm font-bold text-foreground">
        ...
        <Input
          name="tools"
          maxLength={600}
          placeholder={copy.toolsPlaceholder}
          className={fieldClassName}
        />
      </label>
      ...
      <div aria-live="polite" aria-atomic="true" className="min-h-7">
        {status.state === "success" ? (
          <p className="flex items-start gap-3 rounded-2xl border border-accent/30 bg-accent/10 p-4 text-sm leading-6 text-foreground">
            ...
            <span>
              {status.message} {copy.reference}: <strong>{status.reference}</strong>.
            </span>
          </p>
        ) : null}
        {status.state === "error" ? (
